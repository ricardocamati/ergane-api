...
public class VendaServiceImpl implements VendaService {
    ...
    public VendaResponse create(String userId, VendaRequest request) {
        if (request.getItens() == null || request.getItens().isEmpty()) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Venda não pode ser processada com carrinho vazio.");
        }
        ...
        List<Produto> products = request.getItens().stream()
                .map(item -> produtoRepository.findByIdAndUsuarioId(item.getProdutoId(), userId)
                        .orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "Produto não encontrado: " + item.getProdutoId())))
                .toList();
        ...
        for (int i = 0; i < products.size(); i++) {
            ...
            if (product.getEstoque() == null || product.getEstoque() < item.getQuantidade()) {
                throw new ApiException(HttpStatus.BAD_REQUEST, "Estoque insuficiente para o produto: " + product.getNome());
            }
        }
